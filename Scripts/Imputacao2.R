...
covid_full <- read_dta("covid_graficos.dta")
...
covid.miss <- subset(covid_full, select = -c(escolaridade, result_teste_1))
...
imputed_Data <- mice(covid.miss, m=5, maxit = 20, method = 'cart', seed = 500)
...
col <- rep(c("blue", "red")[1 + as.numeric(is.na(imputed_Data$data$contat_cov))], 22)
